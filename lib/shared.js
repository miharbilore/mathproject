...
function createUniqueSuffix() {
  return `${Date.now()}-${process.pid}-${crypto.randomUUID()}`;
}
...
function writeJsonAtomic(filePath, data) {
  ...
  const tempPath = path.join(dir, `.tmp-${createUniqueSuffix()}.json`);
  ...
  try {
    ...
    try {
      ...
      console.warn(`⚠️ Failed to clean temp file ${tempPath}: ${cleanupError.message}`);
    }
    throw error;
  }
}
